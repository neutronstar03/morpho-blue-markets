...
import { onRequestPost as analyticsProxyPost, onRequest as analyticsProxyReject } from './functions/ev'
...
const DEV_API_HANDLERS: Record<string, DevApiRoute> = {
  ...
  '/ev': {
    GET: analyticsProxyReject as unknown as PagesFunctionHandler,
    OPTIONS: analyticsProxyReject as unknown as PagesFunctionHandler,
    POST: analyticsProxyPost as unknown as PagesFunctionHandler,
    PUT: analyticsProxyReject as unknown as PagesFunctionHandler,
  },
}
...
const devPagesEnv: Record<string, unknown> = {
  UMAMI_BACKEND_URL: devVars.UMAMI_BACKEND_URL ?? process.env.UMAMI_BACKEND_URL ?? '',
  USER_BLACKLIST: {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (artifactNames.has(url.pathname)) {
          ...
          if (!existsSync(publicPath)) {
            return writeNodeResponse(res, new Response(JSON.stringify({ error: 'Local artifact not found' }), {
              status: 404,
        ...
        if (!route)
          return next()
        if (!handler) {
          return writeNodeResponse(res, new Response(JSON.stringify({ error: 'Method not allowed' }), {
            status: 405,
            headers: { 'Content-Type': 'application/json' },
          }))
        }
        ...
        try {
          ...
          return writeNodeResponse(res, new Response(JSON.stringify({ error: 'Local Pages Function error' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
          }))
        }
